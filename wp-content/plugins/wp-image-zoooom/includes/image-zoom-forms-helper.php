...
class ImageZoooom_FormsHelper {
    ...
    public function radio($args = array()) {
        ...
        ?>
           <div class="form-group">
             <label class="<?php echo $this->label_class ?>"><?php echo $args['label'] ?></label>
       	      <div class="col-lg-8">
                <?php foreach ($args['values'] as $_id => $_label) : ?>
				<div class="radio">
				  <label>
                  <input type="radio" name="<?php echo $args['name'] ?>" id="<?php echo $_id ?>" value="<?php echo $_id ?>" <?php echo ($_id == $args['active']) ? 'checked=""' : '' ?>>
                  <?php echo $_label ?>
				  </label>
				</div>
                <?php endforeach; ?>
    ...
    public function input_text( $args = array() ) {
        ...
        ?>
        	<div class="form-group">
            <label for="<?php echo $args['name'] ?>" class="<?php echo $this->label_class ?>"><?php echo $args['label'] ?> <?php echo $this->tooltip($args['description']) ?></label>
            <?php if (isset($args['post_input'])) : ?>
                <div class="input-group">
            <?php else : ?>
                <div class="input-group">
            <?php endif; ?>
        <input type="text" class="form-control" id="<?php echo $args['name']?>" name="<?php echo $args['name'] ?>" value="<?php echo $args['value'] ?>" />
    ...
    public function input_color( $args = array() ) {
        ...
        ?>
            <div class="form-group">
            <label for="<?php echo $args['name'] ?>" class="<?php echo $this->label_class ?>"><?php echo $args['label'] ?></label>
				<div class="input-group">
    ...
    public function checkbox( $args = array() ) {
        ...
        ?>
            ...
            <label for="<?php echo $args['name'] ?>" class="<?php echo $this->label_class ?>"><?php echo $args['label'] ?></label>
                  <div class="input-group">
                    <label>
                    <input type="checkbox" id="<?php echo $args['name'] ?>" name="<?php echo $args['name'] ?>" <?php echo ($args['value'] == true) ? 'checked=""' : '' ?> />
                    </label>
                   </div>
            </div>
        <?php
